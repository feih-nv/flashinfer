...
class _CutlassRunnerBase(MoERunner):
    ...
    def pack_inputs(
        ...
        # when the pair carries one. Per-tensor FP8 dequant is 0-dim; swizzled
        # MXFP8 input_sf is a 1-D buffer resized by ConstraintSpec. Sniffing
        # shape[0] == num_tokens treated a (1,) scale at M=1 as token-dynamic
        # and let autotune replace it with a bucket-sized tensor.
    ...
    def _pack_activation_scale_inputs(
        ...
        if self._act_sf_vec_size is not None or self._x_dtype is torch.float8_e4m3fn:
