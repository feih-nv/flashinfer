...
- **First-class prep.** `TrtllmFp4Config.prepare_weights(...)` / `CuteDslConfig.prepare_weights(...)` / `CutlassNvfp4Config.prepare_weights(...)` (and the other quant-specific `Cutlass*Config.prepare_weights` helpers, backed by `flashinfer/fused_moe/prepare.py`) turn canonical bf16 weights into the native views (C6/C7). Weight views are backend-native: CUTLASS NVFP4 uses swizzled `fp4_quantize` scales, not the TRTLLM shuffle / BlockMajorK path, and CUTLASS FP8 / MXFP8 / W4A8 / Humming likewise keep unshuffled or mixed-input layouts distinct from TRTLLM. The quantized activation payload is not: for NVFP4×NVFP4, MXFP4×MXFP8 and MXFP8×MXFP8 every backend consumes the TRT-LLM canonical `hidden_states_q` / `hidden_states_scale`, so the `prepare_activations` helpers of different backends for one pair are interchangeable. This is a claim about the payload and its block scale only. Routing inputs are matched separately: `routing_input_mode` support is per runner (`supported_routing_modes`; Cake is `UnpackedPrecomputed`-only, CUTLASS and CuTe-DSL `PackedPrecomputed`-only) and `MoELayer.__call__` filters on it. Payload-side exceptions: b12x / cuTile NVFP4 take BF16 activations and quantize in-kernel (see `MoEActivationPack`); the opt-in `QuantConfig(swizzled_scale_factors=True)` selects the flat CUTLASS swizzled MXFP8 `input_sf`, which only the CUTLASS MXFP8 runners consume, so a mixed candidate set must use the (default) linear layout; and CUTLASS per-tensor FP8 still takes a dynamic scalar scale on the pack where TRT-LLM keeps a static one in the weight view (#5294 aligns it). Each quant mode uses its matching `Cutlass*Config` / `Cutlass*Runner`; there is no quant-neutral CUTLASS fallback.
...
- **Fail-fast scope:** each runner declares MMA ``(weight, activation)`` pairs
  ...
  is a ``QuantFormat``. Unsupported combinations are filtered before build, the
  empty-candidate ``MoELayer`` error names ``weight=`` / ``activation=`` /
  ``output=``, and direct-runner calls raise a specific ``NotImplementedError``.
